...
namespace padifi
{
    internal class Program
    {
        ...
        {
            // var fileCustomer = @"C:\Develop\ss-ret\ss-ret\padifi\customers.json"
            var fileCustomer = args.First(x => x.Contains("customers.json"));

            var customers = CustomersLoader.FromFile(fileCustomer);
            
            //the main idea: N routes, N starting points. Merge all together and calc routes
            var varaints = Merge(customers);

            //var fileMap  = @"C:\Develop\ss-ret\ss-ret\padifi\map.json"
